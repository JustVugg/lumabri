...
# as untested the moment it appears — the same rule tests/integration/model_family_test.sh
...
for id in $adapters; do
    ...
    if SPLIT_MODEL_DIR="$dir" SPLIT_ENGINE="$id" SPLIT_MODEL="conf-$id" \
       PORT=$(( 8200 + RANDOM % 200 )) bash ./tests/integration/segment_split_test.sh >"/tmp/conf-$id.log" 2>&1; then
